...
class WirelessChannel(nn.Module):
    ...
    def __init__(self, channel_type='bec', outage=0.1, tx_power=1.0, noise_var=1, device='cuda'):
        ...
        if self.channel_type == 'bec':
            ...
            self.bias = None
        elif 'rayleigh' in self.channel_type:
    ...
    def forward(self, input, wireless=False, scalar_gain=True, return_weight=False):
        if self.channel_type == 'bec':
        ...
        elif self.channel_type == 'rayleigh_zf':
            # separate real and imaginary parts and multiply with channel gain
            original_shape = input.shape
            batch_size = original_shape[0]
            ...
            if wireless:
                h = self.weight.sample(dim)
                n = self.bias.sample(dim)
                y = (h * signal + n) / h
                weight = torch.ones_like(h)
                bias = n / h
            else:
                weight = None
                bias = None
...
def computeRiskCertificates(net, toolarge, pbobj, clamping=True, device='cuda', lambda_var=None, train_loader=None, whole_train=None):
    ...
    net.eval()
    with torch.no_grad():
        # loss_train is the average loss during training
        # loss_empirical is the expected risk after KL inversion
        if toolarge:
            train_obj, kl, loss_ce_empirical, err_01_empirical, loss_ce_train, err_01_train = pbobj.compute_final_stats_risk(
    ...
    return train_obj, loss_ce_empirical, err_01_empirical, kl, loss_ce_train, err_01_train
