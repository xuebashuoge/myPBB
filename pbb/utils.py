...
from pbb.bounds import PBBobj, compute_bec_binomial, compute_bec_spec, compute_rayleigh, compute_network_lipschitz
...
def runexp(name_data, objective, prior_type, model, sigma_prior, pmin, learning_rate, momentum, learning_rate_prior=0.01, momentum_prior=0.95, delta=0.025, layers=9, clamping=True, delta_test=0.01, mc_samples=1000, samples_ensemble=100, kl_penalty=1, initial_lamb=6.0, train_epochs=100, prior_dist='gaussian', verbose=False, device='cuda', prior_epochs=20, dropout_prob=0.2, perc_train=1.0, verbose_test=False, perc_prior=0.2, batch_size=250, channel_type='nochannel', outage=0.1, noise_var=1.0, tx_power=1.0, l_0=2, seed=7, norm_type='frob', certificate_name='compare_mc_wired', channel_penalty=1.0):
    ...
    if channel_type.lower() == 'rayleigh':
        ...
        channel_specs = f'rayleigh-zf-tx{tx_power}-noise{noise_var}'
        wireless = True
    ...
    prior_type_lip  = 'rand'
    if objective != 'channel_norm' and objective != 'channel_gradient':
        try:
    ...
    # import ipdb
    # ipdb.set_trace()
    bound = PBBobj(objective, pmin, classes, delta, delta_test, mc_samples, kl_penalty, device, n_posterior=posterior_n_size, n_bound=bound_n_size, K=K, channel_type=channel_type, outage=outage, tx_power=tx_power, noise_var=noise_var, norm_type=norm_type, dimension=dimension, channel_penalty=channel_penalty)
    ...
    loaded_posterior_successfully = False
    ...
    if os.path.exists(f'{posterior_folder}/posterior_model.pt'):
        ...
        try:
            ...
            K_final = result_posterior['K_final']
            ...
            # if not math.isnan(result_posterior['train_loss'][-1]):
            loaded_posterior_successfully = True
        except (KeyError, json.JSONDecodeError) as e:
            print(f"Failed to load posterior results due to error: {e}. Retraining posterior from scratch.")
            loaded_posterior_successfully = False
    
    if not loaded_posterior_successfully:
        ...
        for epoch in trange(train_epochs):
            ...
            # # with torch.no_grad():
            # #     # train_obj, risk_ce, risk_01, kl, loss_ce_train, err_01_train = computeRiskCertificates(net, toolarge, bound, device=device, lambda_var=lambda_var, train_loader=val_bound, whole_train=val_bound_one_batch)
            ...
            #     # print(f"{objective}, {name_data}, {sigma_prior :.5f}, {pmin :.5f}, {learning_rate :.5f}, {momentum :.5f}, {learning_rate_prior :.5f}, {momentum_prior :.5f}, {kl_penalty : .5f}, {dropout_prob :.5f}, {train_obj :.5f}, {risk_ce :.5f}, {risk_01 :.5f}, {kl :.5f}, {loss_ce_train :.5f}, {err_01_train :.5f}, {stch_loss :.5f}, {stch_err :.5f}, {post_loss :.5f}, {post_err :.5f}, {ens_loss :.5f}, {ens_err :.5f}, {errornet0 :.5f}, {perc_train :.5f}, {perc_prior :.5f}")
            
        # record final kl
        kl_final = net.compute_kl()

        # record final lipschitz constant
        K_final = compute_network_lipschitz(net, train_loader, bound, clamping, K, device)
    ...
    # to handled newly added results
    ...
    if os.path.exists(certificate_file):
        ...
        try:
            ...
            loss_ce_empirical = certificate_results['empirical_nll_loss']
            err_01_empirical = certificate_results['empirical_01_error']
            loss_ce_train = certificate_results['train_nll_loss']
            err_01_train = certificate_results['train_01_error']
            ...
            print(f"Failed to load certificate results due to error: {e}. Recomputing certificates from scratch.")
            loaded_successfully = False
    ...
    if not loaded_successfully:
        ...
        with torch.no_grad():
            ...
            train_obj, loss_ce_empirical, err_01_empirical, _, loss_ce_train, err_01_train = computeRiskCertificates(net, toolarge, bound, clamping, device=device, lambda_var=lambda_var, train_loader=val_bound, whole_train=val_bound_one_batch)
        ...
        certificate_results = {
            'empirical_nll_loss': loss_ce_empirical,
            'empirical_01_error': err_01_empirical,
            'train_nll_loss': loss_ce_train,
            'train_01_error': err_01_train,
            'train_obj': train_obj,
    ...
    if channel_type.lower() == 'bec':
        if norm_type == 'frob':
            channel_term = K_final * compute_bec_binomial(dimension, outage, device=device)
        elif norm_type == 'spec':
            channel_term = K_final * compute_bec_spec(dimension, outage, device=device)
        else:
            raise ValueError("norm_type must be 'frob' or 'spec'")
    elif channel_type.lower() == 'rayleigh':
        channel_term = K_final * compute_rayleigh(tx_power, noise_var, device=device)
    elif channel_type.lower() == 'rayleigh_zf':
        # now only works for scalar case
        snr = tx_power / noise_var
        channel_term = K_final * math.pi / (2 * math.sqrt(snr))
    else:
        channel_term = 0.0
    ...
    k = math.sqrt(n_bound)
    # for now, using the empirical loss after KL inversion
    bound_01_rhs = loss_ce_empirical + k * sigma_sub_G**2 / (2*n_bound) + (kl_final - math.log(delta)) / k + channel_term
    bound_ce_rhs = err_01_empirical + k * sigma_sub_G**2 / (2*n_bound) + (kl_final - math.log(delta)) / k + channel_term
    bound_results = {
        'bound_ce_lhs': stch_loss,
        'bound_01_lhs': stch_err,
        'bound_ce_rhs': bound_ce_rhs.item() if torch.is_tensor(bound_ce_rhs) else bound_ce_rhs,
        'bound_01_rhs': bound_01_rhs.item() if torch.is_tensor(bound_01_rhs) else bound_01_rhs,
    ...
    print(f"Objective, Dataset, Sigma, pmin, LR, momentum, LR_prior, momentum_prior, kl_penalty, dropout, Obj_train, Empirical NLL loss, Empirical 01 error, KL, Train NLL loss, Train 01 error, Stch loss, Stch 01 error, Stch loss MC, Stch 01 error MC, Stch loss wired, Stch 01 error wired, 01 error prior net, channel_term, perc_train, perc_prior")
    print(f"{objective}, {name_data}, {sigma_prior :.5f}, {pmin :.5f}, {learning_rate :.5f}, {momentum :.5f}, {learning_rate_prior :.5f}, {momentum_prior :.5f}, {kl_penalty : .5f}, {dropout_prob :.5f}, {train_obj :.5f}, {loss_ce_empirical :.5f}, {err_01_empirical :.5f}, {kl_final :.5f}, {loss_ce_train :.5f}, {err_01_train :.5f}, {stch_loss :.5f}, {stch_err :.5f}, {stch_loss_mc :.5f}, {stch_err_mc :.5f}, {stch_loss_wired :.5f}, {stch_err_wired :.5f}, {errornet0 :.5f}, {channel_term :.5f}, {perc_train :.5f}, {perc_prior :.5f}")
