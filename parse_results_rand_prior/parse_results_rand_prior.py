...
OUTPUT_DIR = 'parse_results_rand_prior/'
...
def main():
    ...
    for folder_name in os.listdir(RESULTS_DIR):
        ...
        # Filtering
        if config['prior_type'] != 'rand': continue
    ...
    for (model, dataset, channel_type), group_df in groups:
        filename_base = f"comparison_{model}_{dataset}_{channel_type}_rand_prior"
        ...
        sorted_df = group_df.sort_values(
            by=['Sort Param', 'Norm Type', 'Epochs', 'Obj Rank', 'Objective', 'KL Penalty', 'Chan Penalty'], 
